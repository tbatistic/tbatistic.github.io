...
;; Chicken Pox
;; By Tyler Batistic and William Liu

turtles-own
  [ sick?                ;; if true, the turtle is infectious   ;; how many weeks of immunity the turtle has left
...
globals
  [ %infected            ;; what % of the population is infectious
    ...
    chance-recover       ;; the probability that a turtle will recover when sick
    ]
...
to setup-constants
  set lifespan 75 * 52      ;; 50 times 52 weeks = 50 years = 2600 weeks old
  set carrying-capacity 300
  set chance-reproduce 1
  set chance-recover 0.9999625
end

;; update variables for graphs
...
to get-sick ;; turtle procedure for becoming sick
  set sick? true
end

to get-healthy ;; turtle procedure for becoming healthy
  set sick? false
  set sick-time 0
end

to become-immune ;; turtle procedure for gaining immunity
  set sick? false
  set sick-time 0
  set immunity 1
end
...
to get-older ;; turtle procedure for gaining age each tick
...
to recover-or-die ;; turtle procedure for choosing between recovery and death
  if sick-time > 20
    [ ifelse age <= 52 * 15  ;;Differ the death frequency for children and adults
      [ ifelse random-float 1 < 0.005
      [ become-immune ]
      [ die ]]
      [ ifelse random-float 1 < chance-recover
      [ become-immune ]
      [ die ]]
      ]
end
...
to-report immune?  ;;report immunity for graphing
  report immunity > 0
end

to spawn-sick  ;; Function to create more sick people based on a slider and button.
  create-turtles spawn-amount  ;; User can choose amount to spawn with a slider.
    [ setxy random-xcor random-ycor
      set age random lifespan
      set sick-time 0
      set immunity 0
      get-sick  ;;  Make the newly spawned turtle sick
      ]
end

to vaccine  ;;  Randomly vaccinate people based on a slider and button.
  ask n-of vaccine-amount turtles
    [ become-immune ]
end
...
; Modified by Tyler Batistic and William Liu
@#$#@#$#@
GRAPHICS-WINDOW
215
10
...
"" ""
PENS
"Children" 1.0 0 -13345367 true "" "plot count turtles with [ age <= 15 ]"
"Adults" 1.0 0 -955883 true "" "plot count turtles with [ age > 15 ]"
